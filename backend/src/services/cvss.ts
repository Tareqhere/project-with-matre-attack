...
const METRIC_VALUES: Record<string, Record<string, number>> = {
  ...
  PR: {                                               // Privileges Required (scope unchanged)
    N: 0.85, L: 0.62, H: 0.27,
  },
  PR_CHANGED: {                                       // Privileges Required (scope changed)
    N: 0.85, L: 0.68, H: 0.50,
  },
  UI: { N: 0.85, R: 0.62 },                         // User Interaction
  S:  { U: 'unchanged', C: 'changed' } as any,      // Scope
...
const SEVERITY_RANGES: Array<{ max: number; label: string }> = [
  { max: 0.0, label: 'None' },
  { max: 3.9, label: 'Low' },
 ...
 */
export function computeCvssScore(vector: string): CvssResult {
  try {
...
export function getSeverityLabel(score: number): string {
  ...
  for (const range of SEVERITY_RANGES) {
    if (score <= range.max) return range.label;
  }
  return 'Critical';
}
 ...
 */
export function estimateScoreFromLabel(label: string): CvssResult {
