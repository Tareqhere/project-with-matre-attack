...
router.get('/', (req: Request, res: Response) => {
  try {
    ...
    if (req.userId) {
      ...
      reports = dbAll(
        'SELECT id, input_type, content_preview, result_json, created_at FROM reports WHERE user_id = ? ORDER BY created_at DESC',
        [req.userId]
      ) as ReportRow[];
    } else if (req.sessionId) {
      // Anonymous user: get session reports
      reports = dbAll(
        'SELECT id, input_type, content_preview, result_json, created_at FROM reports WHERE session_id = ? AND user_id IS NULL ORDER BY created_at DESC',
        [req.sessionId]
      ) as ReportRow[];
    } else {
      reports = [];
...
router.get('/:id', (req: Request, res: Response) => {
  try {
    const report = dbGet(
      'SELECT id, user_id, session_id, input_type, content_preview, result_json, created_at FROM reports WHERE id = ?',
      [req.params.id]
    ...
    const isOwner =
      (req.userId && report.user_id === req.userId) ||
...
router.delete('/:id', (req: Request, res: Response) => {
  try {
    const report = dbGet(
      'SELECT id, user_id, session_id FROM reports WHERE id = ?',
      [req.params.id]
    ) as Pick<ReportRow, 'id' | 'user_id' | 'session_id'> | undefined;
